...
// @raycast.title Set Creative T100 Default Output
// @raycast.mode silent
// @raycast.icon 🔉
...
func setAudioOutput(named targetName: String) -> Bool {
    ...
    guard AudioObjectGetPropertyDataSize(AudioObjectID(kAudioObjectSystemObject), &address, 0, nil, &propsize) == 0 else {
        return false
    }
    ...
    guard AudioObjectGetPropertyData(AudioObjectID(kAudioObjectSystemObject), &address, 0, nil, &propsize, &devids) == 0 else {
        return false
    }
